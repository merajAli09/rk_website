...
import { ArrowLeft, ArrowUpRight, Camera, Check, Clock3, MapPin, Phone, X } from "lucide-react";
  ...
  return (
    <PageShell>
      ...
      <main>
        ...
        <section className="branch-info-section section-pad" data-testid="branch-info-section"><div className="page-container branch-info-grid"><div><SectionIntro eyebrow="01 / Your training floor" title={`${branch.name} details.`} copy="Everything you need to find the floor, plan the session and get moving." /><div className="branch-detail-list"><div className="detail-row" data-testid="branch-address-detail"><MapPin /><div><span>Address</span><p>{branch.address.map((line) => <span key={line}>{line}<br /></span>)}</p></div></div><div className="detail-row" data-testid="branch-phone-detail"><Phone /><div><span>Phone numbers</span><div className="phone-list">{branch.phones.map((phone, index) => <a key={phone} href={`tel:${phone}`} data-testid={`branch-phone-link-${index + 1}`}>{phone}</a>)}</div></div></div><div className="detail-row" data-testid="branch-hours-detail"><Clock3 /><div><span>General timings</span><p data-testid="branch-hours-values">{branch.timings.join(" / ")}</p></div></div><div className="detail-row branch-instagram-detail" data-testid="branch-instagram-detail"><Camera /><div><span>Instagram</span><a className="branch-instagram-link" href={branch.instagramUrl} target="_blank" rel="noreferrer" data-testid="branch-instagram-link">Follow {branch.name} <ArrowUpRight /></a></div></div>{branch.ladiesOnly && <div className="detail-row ladies-only-detail" data-testid="asif-nagar-ladies-session"><Clock3 /><div><span data-testid="ladies-session-label">{branch.ladiesOnly.label}</span><p data-testid="ladies-session-time">{branch.ladiesOnly.time}</p><strong data-testid="ladies-trainer-note">{branch.ladiesOnly.trainer}</strong></div></div>}</div></div><div className="map-card" data-testid="branch-map-card"><iframe title={`Map for RK Fitness ${branch.name}`} src={branch.mapEmbedUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade" data-testid={`branch-map-iframe-${branch.slug}`} /><div className="map-footer"><span data-testid="branch-map-label">Find us in {branch.name}</span><a href={branch.directionsUrl} target="_blank" rel="noreferrer" className="text-link" data-testid="branch-directions-link">Get directions <ArrowUpRight /></a></div></div></div></section>
        ...
        {branch.gallery.length > 0 && <section className="branch-gallery-section section-pad" data-testid="branch-gallery-section"><div className="page-container"><SectionIntro eyebrow="03 / Inside this branch" title={`${branch.name}, in focus.`} /><div className={`branch-gallery-grid branch-gallery-grid-${branch.gallery.length}`}>{branch.gallery.map((image, index) => <button type="button" key={image} onClick={() => setLightboxImage(image)} data-testid={`branch-gallery-image-${index + 1}`}><img src={image} alt={`${branch.name} gym interior ${index + 1}`} /><span>View image <ArrowUpRight /></span></button>)}</div></div></section>}

        <section className="branch-cta-section" data-testid="branch-cta-section"><div className="page-container branch-cta-inner"><div><p className="eyebrow">Your next session starts here</p><h2>Ready to train<br /><span>at {branch.name}?</span></h2></div><div className="branch-cta-actions">{branch.phones.map((phone, index) => <a key={phone} href={`tel:${phone}`} className="yellow-button inline-button" data-testid={`branch-call-now-button-${index + 1}`}><Phone /> {phone}</a>)}<a href={branch.directionsUrl} target="_blank" rel="noreferrer" className="outline-button" data-testid="branch-cta-directions-button">Get directions <ArrowUpRight /></a><a href={branch.instagramUrl} target="_blank" rel="noreferrer" className="outline-button" data-testid="branch-instagram-cta"><Camera /> Follow on Instagram</a></div></div></section>
      </main>
      <Footer />
      {lightboxImage && <div className="lightbox" role="dialog" aria-modal="true" data-testid="branch-gallery-lightbox" onClick={() => setLightboxImage(null)}><button type="button" className="lightbox-close" onClick={() => setLightboxImage(null)} aria-label="Close branch photo" data-testid="branch-gallery-lightbox-close"><X /></button><img src={lightboxImage} alt={`Expanded ${branch.name} gym interior`} onClick={(event) => event.stopPropagation()} data-testid="branch-gallery-lightbox-image" /></div>}
    </PageShell>
  );
}